...
AD_KEYWORDS = [
        "adunit", "vast", "impression", "adinfo", "eventtracking", "creative", "adprovider"
]

EXCLUDE_CONTENT_TYPES = [
        "image/", "text/css", "application/javascript", "application/x-javascript", "application/x-font-ttf", "application/font-woff", "application/font-woff2", "application/octet-stream"
]

EXCLUDE_EXTENSIONS = [
        ".ico", ".css", ".js", ".png", ".jpg", ".jpeg", ".gif", ".svg", ".woff", ".ttf", ".avif", ".webp", ".eot", ".otf", ".pdf", ".zip", "webm"
]
...
def is_useless_json(content) -> bool:
    try:
        ...
        if isinstance(parsed, list) and len(parsed) == 1:
            item = parsed[0]
            if item.get("error") is False and isinstance(item.get("data"), list) and not item["data"]:
                return True
...
def is_ad_related(url, content):
    ...
    return any(domain in url for domain in AD_DOMAINS) or any(keyword in content for keyword in AD_KEYWORDS)
